...
if time_div_in_ns != 2 and time_div_in_ns != 5:
...
lecroy.write(r"""vbs 'app.SaveRecall.Waveform.TraceTitle="Trace%i" ' """%(int(args.runNumber)))
...
#	lecroy.write("STORE_SETUP C%i,HDD,AUTO,OFF,FORMAT,BINARY"%ichan)
	#lecroy.write(r"""vbs 'app.SaveRecall.Waveform.SaveFilename="C%i--Trace%i.trc" ' """%(ichan,int(args.runNumber)))
	#lecroy.write(r"""vbs 'app.SaveRecall.Waveform.SaveFile' """)
lecroy.query("ALST?")
end = time.time()
...
#print "Lecroy run number: %s. Renaming to run %s."%(autoRunNum,args.runNumber)
#for chan in range(1,nchan+1):
#	os.rename("/home/daq/LecroyMount/C%iTrace%s.trc" % (chan,autoRunNum), "/home/daq/LecroyMount/C%iTrace%s.trc" % (chan,args.runNumber))
